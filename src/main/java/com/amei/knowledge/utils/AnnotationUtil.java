...
public class AnnotationUtil {
    ...
    //找到相应注解上的类
    public static <A extends Annotation> List<Class<?>> getAnnotationClass(String packagePath,Class<A> annotationClass){
        List<Class<?>> classes = getClasses(packagePath);
        List<Class<?>> controllers = getControllers(classes,annotationClass);
        return controllers;
    }

    //找也用了Controller注解的类
    private static List<Class<?>> controllers;
    public static <A extends Annotation> List<Class<?>> getControllers(List<Class<?>> clsList,Class<A> annotionClass) {
        if (controllers == null) {
            controllers = new ArrayList<>();
            if (clsList != null && clsList.size() > 0) {
                for (Class<?> cls : clsList) {
                    if (cls.getAnnotation(annotionClass) != null) {
